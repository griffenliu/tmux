...
|分类|按键|描述|替换方案|
| ------------- |:-------------|:-------------|:-----|
||C-b	|命令按键前缀 				|使用C-a（和emacs按键冲突）或者C-k进行替换|
||:  	|进入tmux命令行提示			|	|
||?  	|显示所有的键盘绑定			|	|
||t  	|显示时间				|	| 
||~  	|从tmux中显示以前的消息，如果有的话	|	|
...
||D  	|交互模式分离客户端			|	| 
||d  	|分离当前客客户端			|	| 
||r  	|强制重绘连接的客户端			|这个命令将会被覆盖为重新加载配置文件| 
||$  	|重命名当前会话				|	|
||(  	|将客户端切换到以前的会话		|	|
||)  	|将客户端切换到下一个会话		|	|
||L  	|将客户端切换回上一个会话		|	| 
...
||0 to 9|根据选择的索引切换窗口			|	|
||f  	|在打开的窗口交互模式搜索窗口		|命令行提示输入搜索文本，可以是索引也可以是窗口名称	|  
||w  	|交互模式选择当前窗口			|显示窗口列表进行选择	| 
||'  	|交互模式切换窗口			|命令行提示输入索引	|
||,  	|重命名当前窗口				|	|
||&  	|杀死当前窗口				|	|
||i  	|显示当前窗口的一些信息			|	|  
...
||p  	|切换到前一个窗口			|考虑增加-r参数| 
||c  	|创建新窗口				|	| 
||.	|重设当前窗口的索引	|不能选择已经存在的索引，只能选择还没被占用的，可以从0开始，意味着将当前窗口排在第一个	|
||l  	|切换到上一次的窗口			|貌似没有效果？	| 
||C-o	|将当前窗口向前移动 			|效果不明|
||M-n 	|移动到下一个窗口，响铃或激活标记	|	|
||M-p 	|移动到以前的窗口，响铃或激活标记	|	|


||z  	|把当前pane最大化/恢复			|	|  
||"  	|将当前面板纵向分割为上下2个 		|使用_替换|
||%  	|将当前面板横向分割为左右2个 		|使用|替换|
||!  	|将当前面板拆离为一个窗口 		|	|
||;  	|移动到以前激活的面板			|	|
||o  	|切换到下一个面板			|	|  
||q  	|显示面板索引				|	| 
...
||M  	|清除标记的pane				|	| 
||Space |切换到下一个系统默认布局		|	| 
||Up, Down Left, Right|切换到当前面板的上下左右面板|使用自定义的类vi快捷键代替|
...
||C-Up, C-Down C-Left, C-Right|调整面板的大小，每次1个单位||
||M-Up, M-Down M-Left, M-Right|调整面板的大小，每次5个单位||
